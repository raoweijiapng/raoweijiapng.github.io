...
## 九大内置对象
  ...
  |对象名|类型|作用|
  |--|--|--|
  |request|HttpServletRequest|请求对象,包含了请求相关的信息!|
  |response|HttpServletResponse|响应对象,包含了一些用于响应的功能|
  |pageContext|PageContext|页面的上下文,用于获取其它8大内置对象|
  |session|HttpSession|会话对象,用于会话跟踪与状态管理|
  |application|ServletContext|Servlet的上下文,一个应用程序启动中,只会存在一个Servlet上下文,用于多个Servlet之间的数据共享与通信|
  |out|JSPWriter|打印流,用于向响应体中输出数据|
  |config|ServletConfig|Servlet的配置对象,用于初始化一些键值对信息|
  |page|Object,但是在赋值时,使用的是this.所以本质上是当前JSP转换的Servlet类的类型|指当前页面自身|
  |exception|Throwable|当页面的page指令:isErrorPage="true"时,才会存在的一个对象.当别的页面发生异常后,跳转到此页面时,exception是发生的异常对象|

- request内置对象

> 详情请去HttpServletRequest和HttpServletResponse类查看
...
> 详情请去Cookie和Session技术查看

- application内置对象

> 详情请去ServletContext对象查看
